...
**0. 实作经历不能为了工程感补细节**
> NO: 作者只说"串口通信难"，润色时补成"ACK 丢了、超时重传、翻串口日志定位"这类具体排障故事
> OK: 只压缩和顺句已有事实：「串口通信要反复确认，波特率、指令格式和模块状态任何一处没对上，表现出来都像系统突然失灵」
> 判断：硬件型号、协议字段、日志结论、故障原因、排查顺序，只要原文或用户没有给，就不要补。技术文的人味来自真实细节，不来自替作者编一个更像工程师的 Debug 故事。
